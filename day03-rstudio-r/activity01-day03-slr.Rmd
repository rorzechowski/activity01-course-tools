...
#create letter vector
char_vect <- LETTERS[1:17]

#count number of letters after 'D'
...
```{r load-data}
hifi <- readr::read_csv('https://www.openintro.org/data/csv/hfi.csv')
```
...
**ANSWER: This data set has 1458 rows and 123 columns. Each row represents various political, religious, economical, or personal variables as they relate to freedom, reported by a given country in a given year.**
...
```{r hfi-2016}
hifi_2016 <- hifi %>% filter(.$year == "2016")
```
...
**ANSWER: I would use a boxplot to display the `pf_score` data.**
...
**ANSWER: Yes, I would likely use the same plot for these data as well.**
...
```{r univariable-plots}
#pf_score distribution
ggplot(data = hifi_2016, aes(x = factor(year), 
                                   y = pf_score)) + 
  geom_boxplot()

ggplot(data = hifi_2016, aes(x = pf_score)) + 
  geom_histogram(binwidth = 0.2) +
    geom_vline(aes(xintercept=mean(pf_score, na.rm=T)),   
               color="red", linetype="dashed", size=1)

summary(hifi_2016$pf_score)
```

```{r univariable-plots2}
#pf_expression_control distribution
ggplot(data = hifi_2016, aes(x = factor(year), 
                                   y = pf_expression_control)) + 
  geom_boxplot()

ggplot(data = hifi_2016, aes(x = pf_expression_control)) + 
  geom_histogram(binwidth = 0.4) +
    geom_vline(aes(xintercept=mean(pf_expression_control, na.rm=T)),   
               color="red", linetype="dashed", size=1)

summary(hifi_2016$pf_expression_control)
```

4. Comment on each of these two distributions. 
  Be sure to describe their centers, spread, shape, and any potential outliers.

**ANSWER: The `pf_score` data set has a median value of 6.9, a max of 9.4, and a min of 2.2. Based on the boxplot and histogram, we see that there are two potential outliers. As a whole, the distribution seems fairly even, if we remove the few outliers. The `pf_expression_control` data appears to be unimodally distributed with no outliers. This is corroborated by both the boxplot and the histogram.**
...
**ANSWER: I would use a scatterplot to compare the relationship between the two variables.**
...
**ANSWER: Based on the scatterplot of this relationship, I would say that I do feel comfortable using a linear model to predict a personal freedom score. The relationship appears to have a strong, positive relationship and I suspect that a model would corroborate this.**
...
```{r challenge}
cat("The variance and standard deviation of the pf_score for the data are", var(hifi_2016$pf_score),"and", sd(hifi_2016$pf_score), "respectively")
```
